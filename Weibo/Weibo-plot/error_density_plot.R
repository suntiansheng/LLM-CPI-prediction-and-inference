...
for (i in seq_along(y_m_list)) {
  ...
  tilde_y_m[i, ] <- rows$aveScore
}


topic_cols <- c("1", "2", "3", "4", "5", "6", "7", "8", "9", "0")
if (!all(topic_cols %in% names(df_summary))) {
...
y_df <- df_period %>%
  group_by(month) %>%
  summarise(y = mean(cpi_lastmonth_100, na.rm = TRUE), .groups = "drop") %>%
  filter(!is.na(y)) %>%
  arrange(month)

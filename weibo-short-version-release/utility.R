...
select_topics <- function(y, X_m, EOD, add_one = TRUE) {
  ...
  resid <- as.numeric(ar_fit$residuals)
  valid_idx <- which(!is.na(resid))
  if (length(valid_idx) < 2) {
    stop("Not enough non-NA residuals for topic selection.")
  }
  aic_c <- NULL
  for (i in 1:20) {
    strong_idx <- order(abs(cov(resid[valid_idx], X_m[valid_idx, ])), decreasing = TRUE)[1:i]
  ...
  strong_idx <- order(abs(cov(resid[valid_idx], X_m[valid_idx, ])), decreasing = TRUE)[1:select_num]
  strong_idx
}

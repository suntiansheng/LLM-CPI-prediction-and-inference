...
set.seed(1234)
h_max <- 15
train_end <- max(1, length(y) - h_max)
EOD <- min(45, train_end) # select topics using training window only
strong_idx <- select_topics(y[1:train_end], X_m[1:train_end, , drop = FALSE], EOD = EOD, add_one = TRUE)
strong_idx
```
...
# Use an AR fit on the selection window to compute correlations for plotting.
...
data <- sort(abs(cov(resid[valid_idx], X_m[valid_idx, ])))

...
split_phi_mats <- function(Phi_arr, k) {
  if (length(dim(Phi_arr)) == 3) {
    return(lapply(seq_len(dim(Phi_arr)[3]), function(j) as.matrix(Phi_arr[, , j])))
  }
  if (ncol(Phi_arr) %% k != 0) {
    stop("Phi matrix dimensions are incompatible with tilde_y_m.")
  }
  p_use <- ncol(Phi_arr) / k
  lapply(seq_len(p_use), function(j) {
    Phi_arr[, ((j - 1) * k + 1):(j * k), drop = FALSE]
  })
}
...
LLM_TS.Predict <- function(X_m, y, tilde_y_m, p1, p2, obs_idx, test_idx,
  ...
  Phi_list <- split_phi_mats(Phi_arr, ncol(tilde_y_m))
...
select_topics <- function(y, X, train_idx, use_cor = TRUE,
  ...
  if (!use_early_stop && is.finite(max_topics) && max_topics > 0) {
    selected <- head(topic_order, max_topics)
  }
...
select_topics_predictive <- function(y, X, train_idx, max_topics = 2,
                                     min_train = 24, horizon = 1) {
  ar_base <- auto.arima(y[train_idx], max.q = 0, D = 0, seasonal = FALSE,
                        allowmean = FALSE, allowdrift = FALSE)
  p_ar <- ar_base$arma[1]
  ...
  if (max_topics <= 1) {
    combos <- lapply(topic_names, function(x) x)
  } else {
    combos <- combn(topic_names, max_topics, simplify = FALSE)
  }

  eval_combo <- function(cols) {
    n_train <- length(train_idx)
    start_t <- max(min_train, p_ar + 2)
    if (n_train <= start_t) return(Inf)
    errs <- numeric(0)
    for (t in seq(from = start_t, to = n_train - horizon)) {
      idx <- train_idx[1:t]
      next_idx <- train_idx[t + horizon]
      fit <- tryCatch(
        arima(y[idx], order = c(p_ar, 0, 0), xreg = X[idx, cols, drop = FALSE],
              include.mean = FALSE),
        error = function(e) NULL
      )
      if (is.null(fit)) next
      pred <- tryCatch(
        predict(fit, n.ahead = horizon, newxreg = X[next_idx, cols, drop = FALSE])$pred,
        error = function(e) NULL
      )
      if (is.null(pred)) next
      errs <- c(errs, y[next_idx] - pred[horizon])
    }
    if (!length(errs)) return(Inf)
    sqrt(mean(errs^2))
  }

  rmse_vals <- sapply(combos, eval_combo)
  best_idx <- which.min(rmse_vals)
  list(
    selected = combos[[best_idx]],
    ar_order = p_ar,
    scores = data.frame(
      combo = sapply(combos, function(x) paste(x, collapse = ",")),
      rmse = rmse_vals,
      stringsAsFactors = FALSE
    )
  )

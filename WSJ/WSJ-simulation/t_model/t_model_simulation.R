...
topic_cols <- grep("^topic_", names(df), value = TRUE)
df_month <- df %>%
  dplyr::select(month, all_of(topic_cols)) %>%
  group_by(month) %>%
  summarise(across(everything(), \(x) mean(x, na.rm = TRUE)), .groups = "drop")

X_m <- as.matrix(df_month[, topic_cols])
X_m <- scale(X_m)
X_select <- X_m[, c(match("topic_4", topic_cols), match("topic_9", topic_cols))]
...
generate_arx_data <- function(X, rho) {
  ...
  diag(sigma) <- 1
  error_c <- mvtnorm::rmvt(n, sigma = sigma, df = 10)
...
h_c <- seq(8, 15, 1)
strong_idx <- match(c("topic_4", "topic_9"), topic_cols)
p1 <- 2
p2 <- 1
...
for (d in 1:length(rho_c)) {
  ...
  MSE_result <- rbind(MSE_result, t(mse_sum / denom))
  Sign_result <- rbind(Sign_result, t(sign_sum / denom))
}

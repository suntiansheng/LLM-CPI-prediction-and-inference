---
title: "WSJ Data Analysis with LDA Embedding"
date: "2025-12-22"
...
target_list <- c("CPIAUCSL", "CPIAPPSL", "CPITRNSL", "CPIMEDSL", "CPIULFSL")
if (identical(params$target_var, "ALL")) {
  rscript_bin <- Sys.which("Rscript")
  if (!nzchar(rscript_bin)) {
    stop("Rscript not found on PATH; cannot render per-index outputs.")
  }
  wd <- normalizePath(getwd(), winslash = "/")
  wd <- gsub("'", "\\\\'", wd)
  for (idx in target_list) {
    expr <- sprintf(
      "setwd('%s'); rmarkdown::render('LDA.Rmd', params=list(target_var='%s'), output_file='LDA_%s.html')",
      wd, idx, idx
    )
    cmd <- paste0(shQuote(rscript_bin), " -e \"", gsub("\"", "\\\\\"", expr), "\"")
    status <- system(cmd)
    if (!identical(status, 0L)) {
      stop(paste("Render failed for index:", idx))
    }
  }
  knitr::knit_exit()
}
...
topics_df <- read.csv("./topics_with_pred_score_final.csv")
...
for(k in seq_along(h_pi)){
  ...
  boot_lda_res <- LLM_TS.BOOT(X_m, y, tilde_y_m, p1, p2, obs_idx, test_idx, strong_idx, h, B = 2000)
  ...
  if (has_unem) {
    boot_res <- LLM_TS.BOOT(cbind(X_m,unem), y, tilde_y_m, p1, p2, obs_idx, test_idx, c(strong_idx,ncol(X_m)+1), h, B = 2000)

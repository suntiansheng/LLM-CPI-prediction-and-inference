...
generate_arx_data <- function(X, rho) {
  ...
  error_c <- 0.1*mvtnorm::rmvt(n, sigma = sigma, df=10)
...
h_c <- seq(8,15,1)
#h_c <- seq(2,9,1)
strong_idx <- c(11,12) #know the true index
p1=2
p2=1
...
write.csv(MSE_result, './result/t_distribution_MSE_result.csv', row.names = FALSE)
write.csv(Sign_result, './result/t_distribution_Sign_result.csv', row.names = FALSE)
MSE_result
Sign_result